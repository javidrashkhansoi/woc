...
const MAX_WIDTH_1399PX = matchMedia("(max-width: 1399px)");
const MAX_WIDTH_992PX = matchMedia("(max-width: 992px)");
const MAX_WIDTH_768PX = matchMedia("(max-width: 768px)");
const constants_root = document.documentElement;
const body = document.body;

;// CONCATENATED MODULE: ./src/js/modules/modules/padding-right-action.js


function paddingRightAction(padding = true) {
  const wrapper = document.querySelector(".wrapper");
  const lockPaddingItems = document.querySelectorAll(".lock-padding");
  const lockPaddingValue = padding ? `${innerWidth - wrapper?.offsetWidth}px` : "";

  body.style.paddingInlineEnd = lockPaddingValue;

  lockPaddingItems?.forEach(lockPaddingItem => {
    lockPaddingItem.style.paddingInlineEnd = lockPaddingValue;
  });

  constants_root.classList.toggle("lock", padding);
}

;// CONCATENATED MODULE: ./src/js/modules/modules/burger-action.js
...
function burgerToggle() {
  constants_root.classList.toggle(burgerActiveClass);

  paddingRightAction(isBurgerActive());
}

function burger_action_burgerClose() {
  if (isBurgerActive()) burgerToggle();
}

function isBurgerActive() {
  return constants_root.classList.contains(burgerActiveClass);
}

;// CONCATENATED MODULE: ./src/js/modules/modules/go-to.js



/** @param {Element} element */
function goToAction(element) {
  /** @type {string} */
  const elementData = element.dataset.goto;
  const searchedBlock = document.querySelector(elementData);

  if (searchedBlock) {
    const headerWrapperBlockSize = parseFloat(root.style.getPropertyValue("--hwbs").trim());
    const searchedBlockTopPosition = searchedBlock.getBoundingClientRect().top + scrollY - headerWrapperBlockSize;

    burgerClose();

    scrollTo({
      top: searchedBlockTopPosition,
      behavior: "smooth",
    });
  }
}
...
const filter = document.querySelector(".catalog-aside");
const filterDragger = document.querySelector(".catalog-filter-form__dragger");

function catalogFilterToggle() {
  if (filter) {
    filter.classList.toggle("show");
    constants_root.classList.toggle("filter-active");
    paddingRightAction(filter.classList.contains("show"));

    if (filterDragger) {
      filterDragger.addEventListener("touchstart", filterTouchStart);
    }
  }
}
...
function catalogFilterHide() {
  if (filter) {
    ...
    if (filterDragger) {
      filterDragger.removeEventListener("touchstart", filterTouchStart);
      filterDragger.removeEventListener("touchmove", filterMove);
      filterDragger.removeEventListener("touchend", filterTouchEnd);
    }

    setTimeout(() => {
      constants_root.classList.remove("filter-active");
...
function filterTouchStart(event) {
  const { touches } = event;
  const { clientY } = touches[0];

  start = clientY;

  filterDragger.addEventListener("touchmove", filterMove);
  filterDragger.addEventListener("touchend", filterTouchEnd, { once: true });
}

function filterMove(event) {
  const { touches } = event;
  const { clientY } = touches[0];

  difference = -1 * (start - clientY);

  if (difference > 0) {
    filter.style.insetBlockStart = `${difference + 40}px`;
  }
}

function filterTouchEnd() {
  if (difference > 100) {
    catalogFilterHide();

    setTimeout(() => {
      filter.style.removeProperty("inset-block-start");
    }, 300);
  } else {
    filter.style.removeProperty("inset-block-start");
  }
...
;// CONCATENATED MODULE: ./src/js/scripts/scripts/description-toggle.js
function toggleDescription(button) {
  const section = button.closest(".description");
  const inner = section?.querySelector(".description__inner");

  if (inner) {
    button.classList.toggle("active");
    inner.classList.add("show");

    const innerScrollSize = inner.scrollHeight;

    if (button.classList.contains("active")) {
      inner.style.maxBlockSize = `${innerScrollSize}px`;

      setTimeout(() => {
        inner.style.maxBlockSize = "fit-content";
      }, 300);
    } else {
      inner.style.maxBlockSize = `${innerScrollSize}px`;

      setTimeout(() => {
        inner.style.maxBlockSize = "";
      });
...
;// CONCATENATED MODULE: ./src/js/scripts/scripts/dialog-action.js



function showDialog(button) {
  const buttonData = button.dataset.dialogTarget;
  const dialog = document.querySelector(`[data-dialog="${buttonData}"]`);

  if (dialog) {
    constants_root.classList.add("dialog-active");
    paddingRightAction();
    dialog.show();
  }
}
...
function closeDialog(button) {
  ...
  if (dialog) {
    constants_root.classList.remove("dialog-active");
    paddingRightAction(false);
    dialog.close();
  }
}
...
function closeAllDialogs() {
  const activeDialogs = document.querySelectorAll("[data-dialog][open]");
...
;// CONCATENATED MODULE: ./src/js/scripts/scripts/go-to-top.js
function goToTop() {
  scrollTo({
    top: 0,
    behavior: "smooth",
...
;// CONCATENATED MODULE: ./src/js/modules/modules/slide-up-and-down.js
...
;// CONCATENATED MODULE: ./src/js/scripts/scripts/header-products-tab-action.js
...
function headerProductsTabAction(button) {
  const buttonData = button.dataset.productsTab;
  const buttonOrder = +button.style.getPropertyValue("--header-products-tab-order").trim();
  const tabsBlock = button.closest("[data-tabs]");
  const tabs = tabsBlock?.querySelectorAll("[data-products-tab]");
  const panelsBlock = tabsBlock?.querySelector("[data-panels]");
  const panels = panelsBlock?.querySelectorAll("[data-panel]");
  const tabsBlockInner = tabsBlock?.querySelector(".header-products__inner");

  let tabsBlockInnerRowGap;

  if (tabsBlockInner) {
    tabsBlockInnerRowGap = parseInt(getComputedStyle(tabsBlockInner).rowGap);
  }

  if (order === undefined) order = buttonOrder;

  if (panels && Array.from(panels).every(panel => !panel.classList.contains("slide"))) {
    panelsBlock.hidden = false;

    tabs?.forEach(tab => {
      tab.classList.toggle("active", tab.dataset.productsTab === buttonData);
    });
    ...
    if (tabsBlockInner && order !== buttonOrder) {
      tabsBlockInner.style.rowGap = `${tabsBlockInnerRowGap / 2}px`;
      order = buttonOrder;
    }

    setTimeout(() => {
      panels.forEach(panel => {
        if (panel.dataset.panel === buttonData) {
          slideDown(panel);
        }
      });

      panelsBlock.style.setProperty("--header-products-panels-order", buttonOrder ? buttonOrder + 1 : 1);
      tabsBlockInner?.style.removeProperty("row-gap");
...
;// CONCATENATED MODULE: ./src/js/scripts/scripts/remove-header-warning.js
function removeHeaderWarning() {
  const headerWarning = document.querySelector(".header-warning");

  headerWarning?.remove();
}

;// CONCATENATED MODULE: ./src/js/scripts/scripts/show-more-categories.js


function showMoreCategories(button) {
  const section = button.closest(".categories");
  const hiddenCategories = section?.querySelector(".categories__column_hidden");

  if (hiddenCategories) {
    slideDown(hiddenCategories);
    button.remove();
  }
}

;// CONCATENATED MODULE: ./src/js/scripts/scripts/stores-toggle.js


function storesToggle(button) {
  const section = button.closest(".stores");

  if (section) {
    const buttonsPreview = button.closest(".stores-preview");

    if (buttonsPreview && !section.classList.contains("show")) {
      const buttonsPreviewBottom = buttonsPreview.getBoundingClientRect().bottom;

      section.style.setProperty("--stores-preview-bottom-position", `${buttonsPreviewBottom}px`);
    }

    section.classList.toggle("show");

    paddingRightAction(section.classList.contains("show"));
  }
}

function hideStores(unlock = true) {
  const section = document.querySelector(".stores");

  if (section) {
    section.classList.remove("show");

    paddingRightAction(!unlock);
  }
}

function changePreviewInner(currentButton) {
  const buttonInner = currentButton.querySelector(".stores-button__inner");
  const section = currentButton.closest(".stores");
  const previewInner = section?.querySelector(".stores-preview .stores-button__inner");

  if (buttonInner && previewInner) {
    const buttons = section.querySelectorAll(".stores-button");

    previewInner.outerHTML = buttonInner.outerHTML;
...
;// CONCATENATED MODULE: ./src/js/scripts/scripts/toggle-header-products-tabs.js


function showHeaderProductsTab(button) {
  const tabs = document.querySelector(".header-products");

  button.classList.toggle("active");
  tabs?.classList.toggle("show");

  paddingRightAction(button.classList.contains("active"));
}

function hideHeaderProductsTab() {
  const button = document.querySelector(".header-buttons__button");
  const tabs = document.querySelector(".header-products");
...
;// CONCATENATED MODULE: ./src/js/scripts/scripts/toggle-header-tels.js


function showHeaderTels(button) {
  const tels = document.querySelector(".header-phone__inner");

  button.classList.toggle("active");
  tels?.classList.toggle("show");
  burger_action_burgerClose();
}

function hideHeaderTels() {
  const button = document.querySelector(".header-phone__show");
  const tels = document.querySelector(".header-phone__inner");

  button?.classList.remove("active");
  tels?.classList.remove("show");
}

;// CONCATENATED MODULE: ./src/js/scripts/scripts/toggle-search-from.js
...
function hideSearchForm() {
  const button = document.querySelector(".header-buttons__search");
  const form = document.querySelector(".header-form");
...
;// CONCATENATED MODULE: ./src/js/events/events/click.js
...
function click(event) {
  ...
  //   goToAction(target.closest("[data-goto]"));
  // }

  if (target.closest(".header-warning__close")) {
    removeHeaderWarning();
  }

  if (target.closest(".header-buttons__search")) {
    showSearchForm(target.closest(".header-buttons__search"));
  } else if (!target.closest(".header-form")) {
    hideSearchForm();
  }

  if (target.closest(".header-phone__show")) {
    showHeaderTels(target.closest(".header-phone__show"));
  } else if (!target.closest(".header-phone__inner")) {
    hideHeaderTels();
  }

  if (target.closest(".header-buttons__button")) {
    showHeaderProductsTab(target.closest(".header-buttons__button"))
  } else if (!target.closest(".header-products") && !MAX_WIDTH_1399PX.matches) {
    hideHeaderProductsTab();
  }

  if (target.closest("[data-products-tab]")) {
    headerProductsTabAction(target.closest("[data-products-tab]"));
  }

  if (target.closest(".header-arrow__button")) {
    goToTop();
  }

  if (target.closest("[data-dialog-target]")) {
    showDialog(target.closest("[data-dialog-target]"));
  } else if (target.closest("[data-dialog-close]") || (!target.closest("[data-dialog-inner]") && target.closest("[data-dialog]"))) {
    closeDialog(target.closest("[data-dialog-close]") || (!target.closest("[data-dialog-inner]") && target.closest("[data-dialog]")));
  }

  if (target.closest(".categories__button")) {
    showMoreCategories(target.closest(".categories__button"));
  }

  if (target.closest(".description__button")) {
    toggleDescription(target.closest(".description__button"));
  }

  if (target.closest(".stores-preview__button")) {
    storesToggle(target.closest(".stores-preview__button"));
  } else if (!target.closest(".stores-buttons")) {
    hideStores(!constants_root.classList.contains("burger-active"));
  }

  if (target.closest(".stores-button")) {
    changePreviewInner(target.closest(".stores-button"));
  }

  if (event.target.closest(".catalog-sort-form__button")) {
    catalogFilterToggle();
  } else if (!event.target.closest(".catalog-aside") && constants_root.classList.contains("filter-active")) {
    catalogFilterHide();
  }
}

;// CONCATENATED MODULE: ./src/js/events/events/keydown.js
...
;// CONCATENATED MODULE: ./src/js/events/events/max-1399-event.js



/** @param {MediaQueryListEvent} event */
function max1399event(event) {
  const { matches } = event;
...
;// CONCATENATED MODULE: ./src/js/events/events/max-992-event.js



/** @param {MediaQueryListEvent} event */
function max992event(event) {
  const { matches } = event;
...
;// CONCATENATED MODULE: ./src/js/events/events/scroll.js
function scroll_scroll(event) {
  const anchorsBlock = document.querySelector(".shipping-paymant-anchors__block");

  if (anchorsBlock) {
    anchorsBlock.style.setProperty("--scroll-y", `${scrollY}px`);
  }
}

;// CONCATENATED MODULE: ./src/js/events/events.js
...
addEventListener("scroll", scroll_scroll);

MAX_WIDTH_1399PX.addEventListener("change", max1399event);
MAX_WIDTH_992PX.addEventListener("change", max992event);
// MAX_WIDTH_768PX.addEventListener("change", max768event);

document.addEventListener("DOMContentLoaded", scroll_scroll);
document.addEventListener("click", click);
document.addEventListener("keydown", keydown);

;// CONCATENATED MODULE: ./src/js/observers/intersection/observers/header-observer.js
...
const headerIntersectionObserver = new IntersectionObserver(entries => {
  entries.forEach(entry => {
    ...
    constants_root.classList.toggle("scrolled", !isIntersecting);
  });
});

const header = document.querySelector(".header");

if (header) headerIntersectionObserver.observe(header);

;// CONCATENATED MODULE: ./src/js/observers/intersection/observers/catalog-observer.js
const catalogInnerObserver = new IntersectionObserver(entries => {
  entries.forEach(entry => {
...
if (catalogInner) catalogInnerObserver.observe(catalogInner);

const fixedItemsObserver = new IntersectionObserver(entries => {
  entries.forEach(entry => {
    const { target, isIntersecting, boundingClientRect } = entry;
    const { top } = boundingClientRect;

    target.classList.toggle("fixed", !isIntersecting && top < 0);
  });
});

const fixedItems = document.querySelectorAll(".catalog-inner__sort, .shipping-paymant-anchors__block");

fixedItems?.forEach(fixedItem => {
  fixedItemsObserver.observe(fixedItem);
});

;// CONCATENATED MODULE: ./src/js/observers/intersection/intersection.js



;// CONCATENATED MODULE: ./src/js/observers/resize/observers/header-observer.js


const headerResizeObserver = new ResizeObserver(entries => {
  entries.forEach(entry => {
    const headerWrapper = entry.target;
    const headerWrapperBlockSize = getComputedStyle(headerWrapper).blockSize;
    const headerWarning = headerWrapper.querySelector(".header-warning");

    constants_root.style.setProperty("--header-wrapper-block-size", headerWrapperBlockSize);

    if (headerWarning) {
      const headerWarningBlockSize = getComputedStyle(headerWarning).blockSize;

      constants_root.style.setProperty("--header-warning-block-size", headerWarningBlockSize);
    } else {
      constants_root.style.removeProperty("--header-warning-block-size");
    }
  });
});
...
if (headerWrapper) headerResizeObserver.observe(headerWrapper);

;// CONCATENATED MODULE: ./src/js/observers/resize/observers/same.js
const sameResizeObserver = new ResizeObserver(entries => {
  entries.forEach(entry => {
    /** @type {HTMLElement} */
    const same = entry.target;
    /** @type {HTMLElement} */
    const samesBlock = same.closest("[data-sames]");
    /** @type {NodeListOf<HTMLElement>} */
    const sames = samesBlock.querySelectorAll("[data-same]");

    sames.forEach(same => {
      same.style.removeProperty("--same-block-size");
    });

    const samesArray = Array.from(sames);
    const samesBlockSizeArray = samesArray.map(same => parseFloat(getComputedStyle(same).blockSize));
    const maxSameBlockSize = Math.max(...samesBlockSizeArray);

    sames.forEach(same => {
      same.style.setProperty("--same-block-size", maxSameBlockSize || 0);
    });
  });
});

/** @type {NodeListOf<HTMLElement>} */
const samesBlocks = document.querySelectorAll("[data-sames]");

samesBlocks?.forEach(samesBlock => {
  /** @type {NodeListOf<HTMLElement>} */
  const sames = samesBlock.querySelectorAll("[data-same]");

  sames?.forEach(same => {
    sameResizeObserver.observe(same);
  });
});

;// CONCATENATED MODULE: ./src/js/observers/resize/observers/header-products-observer.js
let inlineSize;

const headerProductsResizeObserver = new ResizeObserver(entries => {
  entries.forEach(entry => {
    const { target } = entry;
    const targetInlineSize = parseFloat(getComputedStyle(target).inlineSize);
    const targetColumnGap = parseInt(getComputedStyle(target).columnGap);
    const tabs = target.querySelectorAll("[data-products-tab]");
    const panels = target.querySelector("[data-panels]");

    let sum = targetInlineSize;
    let cheker = 0;

    if (!inlineSize) inlineSize = targetInlineSize;

    if (tabs.length) {
      tabs.forEach(tab => {
        tab.style.removeProperty("--header-products-tab-order");
      });

      for (const tab of tabs) {
        const tabInlineSize = parseFloat(getComputedStyle(tab).inlineSize);

        if (tabInlineSize > sum) {
          sum = targetInlineSize;
          cheker++;
        }

        tab.style.setProperty("--header-products-tab-order", cheker * 2);

        if (tab.classList.contains("active") && inlineSize !== targetInlineSize) {
          panels?.style.setProperty("--header-products-panels-order", cheker * 2 + 1);
          inlineSize = targetInlineSize;
        };

        sum -= tabInlineSize + targetColumnGap;
      }
    }
  });
});

const headerProducts = document.querySelector(".header-products__inner");

if (headerProducts) headerProductsResizeObserver.observe(headerProducts);

;// CONCATENATED MODULE: ./src/js/observers/resize/resize.js




;// CONCATENATED MODULE: ./src/js/observers/observers.js
...
;// CONCATENATED MODULE: ./src/js/libraries/swiper/sliders/products-preview.js


const productsPreviewSections = document.querySelectorAll(".products-preview");

productsPreviewSections?.forEach(section => {
  const slider = section.querySelector(".products-slider");
  const next = section.querySelector(".products-arrows__next");
  const prev = section.querySelector(".products-arrows__prev");

  if (slider && next && prev) {
    const productsPreviewSlider = new core(slider, {
...
;// CONCATENATED MODULE: ./src/js/libraries/swiper/sliders/banner.js


const bannerSlider = new core(".banner-slider", {
  modules: [Navigation, Keyboard,],
  navigation: {
    nextEl: ".arrows-banner__arrow_next",
    prevEl: ".arrows-banner__arrow_prev",
  },
  keyboard: {
    enabled: true,
    onlyInViewport: true,
...
;// CONCATENATED MODULE: ./src/js/libraries/swiper/sliders/products-add.js


const productsPreviewSlider = new core(".products-add-slider", {
  modules: [Pagination, Keyboard,],
  pagination: {
    el: ".products-add-slider__pagination",
    type: "bullets",
    clickable: true,
    dynamicBullets: false,
  },
...
;// CONCATENATED MODULE: ./src/js/libraries/swiper/sliders/product.js


const productSlider = new core(".product-slider", {
  modules: [Keyboard, Thumb,],
  keyboard: {
    enabled: true,
    onlyInViewport: true,
    pageUpDown: false,
...
;// CONCATENATED MODULE: ./src/js/libraries/nouislider/ranges/catalog-range.js


const catalogRange = document.getElementById("catalog-range");

if (catalogRange) {
  const { min, max, start, end } = catalogRange.dataset;

  if (min && max && start && end) {
    const inputFrom = document.getElementById("catalog-range-from");
    const inputTo = document.getElementById("catalog-range-to");
    ...
    if (inputFrom && inputTo) {
      ...
      inputs.forEach((input, handle) => {
        input.addEventListener('change', function () {
          catalogRange.noUiSlider.setHandle(handle, this.value);
        });
      });
    }
  }
...
;// CONCATENATED MODULE: ./src/js/libraries/nouislider/nouislider.js
...
;// CONCATENATED MODULE: ./src/js/libraries/choices/selects/product-count.js



const productCountSelect = document.querySelector(".product-form__select select");

if (productCountSelect) {
  const productCountSelect = new choices(".product-form__select select", {
    searchEnabled: false,
    itemSelectText: "",
    position: "bottom",
  });

  productCountSelect.passedElement.element.addEventListener("showDropdown", function () {
    const productCountSelectDropdownList = document.querySelector(".product-form__select .choices__list--dropdown > .choices__list");

    const productCountScrollbar = new SimpleBar(productCountSelectDropdownList);
  });
}

;// CONCATENATED MODULE: ./src/js/libraries/choices/selects/catalog-sort.js


const sort = document.querySelector(".catalog-sort-form__select select");
...
;// CONCATENATED MODULE: ./src/js/modules/modules/dynamic-adaptive.js
class DynamicAdapt {
  constructor(type) {
    this.type = type;
  }
...
;// CONCATENATED MODULE: ./src/js/modules/modules/spollers.js
...
;// CONCATENATED MODULE: ./src/js/modules/modules.js
// import "./modules/burger-action.js";
// import "./modules/custom-select.js";

// import "./modules/form.js";
// import "./modules/go-to.js";
// import "./modules/is-browser-supports-webp.js";
// import "./modules/is-touch-screen.js";
// import "./modules/lazy-images.js";
// import "./modules/padding-right-action.js";
// import "./modules/slide-up-and-down.js";
